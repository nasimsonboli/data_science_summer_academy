...
%% Create 40 class classifier using fitcknn 
faceClassifier = fitcknn(trainingFeatures,trainingLabel, 'NumNeighbors',1);
...
for person=1:5
    for j = 1:test(person).Count
        ...
        booleanIndex = strcmp(personLabel, personIndex);
        integerIndex = find(booleanIndex);
        subplot(test(person).Count,2,figureNum);
        imshow(imresize(queryImage,3));
        title('Query Face');
        subplot(test(person).Count,2,figureNum+1);imshow(imresize(read(training(integerIndex),1),3));title('Matched Class');
